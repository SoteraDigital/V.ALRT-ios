...
@implementation DevicoffpopView
...
+ (DevicoffpopView *) sharedInstance{
    ...
    static DevicoffpopView *myInstance = nil;
...
-(id)initWithView
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        ...
        arrayOfViews = [[NSBundle mainBundle] loadNibNamed:@"DevicoffpopView"
                                                     owner:nil
                                                   options:nil];
    }
    ...
    DevicoffpopView *newView = [arrayOfViews objectAtIndex:0];
...
-(void)didConfirmationViewLoad:(UIView *)parentView andConfirmationViewTitle:(NSString *)strConfirmationViewTitle andConfirmationViewContent:(NSString *)strConfirmationViewContent andConfirmationViewCallback:(OnConfirm)callback{
    ...
    [noBtn setTitle:NSLocalizedString(@"cancel", nil) forState:UIControlStateNormal];
    [yesBtn setTitle:NSLocalizedString(@"turn_off", nil) forState:UIControlStateNormal];
    ...
    if([strConfirmationViewTitle isEqualToString:@""])
    {
        lblContent.text = TEXT_ADD_CONTACT;
    }
    else
    {
    ...
    float y = (self.window.frame.size.height-130)/2;
